function [IFest, IAest, sest] = ACMD1(s,fs,eIF,alpha0,tol)
% Adaptive Chirp Mode Decomposition (ACMD)
% Authors: Shiqian Chen 
% mailto:chenshiqian@swjtu.edu.cn; 
% https://www.researchgate.net/profile/Shiqian_Chen2  
% In this code, the IF is assumed to be constant; the algorithm can decompose a signal into different frequency bands
%
%%%%%%%%%%%%%%%%%%%%%%%  input %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% alpha0: penalty parameter controling the filtering bandwidth of ACMD;the smaller the alpha0 is, the narrower the bandwidth would be (the signal will be more smooth)
...
%%%%%%%%%%%%%%%%update demodulated signals%%%%%%%%%%%%%%%%%%%%%%%%%%%5
 ...
 %   deltaIF = (1/beta*opedoub + speye(N))\deltaIF';% smooth the frequency increment by low pass filtering
    eIF = eIF - mean(deltaIF);% update the IF; the IF is constant
    IFsetiter(iter,:) = eIF;    
